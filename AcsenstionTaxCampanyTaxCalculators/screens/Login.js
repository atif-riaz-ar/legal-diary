...
const Login = ({ navigation }) => {
  ...
  const SocialLoginTrigger = (data) => {
    setLoading(true);
    fetch("https://leedsng.com/api/login_social.php", {
      method: "POST",
      body: data,
    }).then((responseData) => {
  ...
  GoogleSignin.configure({
    webClientId: "716980723963-s831o52gi9756df48u9b1dc3ggjip8rp.apps.googleusercontent.com",
    offlineAccess: true,
  });
  ...
  const handleSubmitPress = () => {
    ...
    fetch("https://leedsng.com/api/login.php", {
      method: "POST",
      body: data,
    }).then((responseData) => {
      return responseData.json();
